...
async function main() {
  ...
  sepBridgeContract.on("Transfer", async (from, to, amount, date, nonce, signature) => {
    console.log("Hoooorrrraaay, We've caught the Event for Sepolia to Polygon...!");
  ...
  polBridgeContract.on("Transfer", async (from, to, amount, date, nonce, signature) => {
    console.log("Hoooorrrraaay, We've caught the Event for Polygon to Sepolia...!");
    ...
    const data = sepBridgeContract.interface.encodeFunctionData('mint', [from, to, amount, nonce, signature]);
    // to, uint amount, uint nonce, bytes calldata signature 
    let _gasCost = await sepBridgeContract.estimateGas.mint(from, to, amount, nonce, signature);
    ...
    console.log(`DEC POLYGON-ETH Transaction hash: ${receipt.hash}`,"\n","CTRL+C for exit..","\n","\n","\n","\n","\n");
    return 0;
  });
